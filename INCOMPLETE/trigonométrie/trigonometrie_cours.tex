...
\begin{center}
    \begin{tikzpicture}[scale=2]
        ...
        % Point M
        \node[left] at (0.707,0.707) {\scriptsize{$M$}};
        \draw[mark size=1.5pt,mark=+] plot coordinates {(0.707,0.707)};
        % Droite
        \draw[red, thick] (1,1.4) -- (1,-1.2);
        \draw[red, thick, dashed] (0.707,0.707) -- (1,0.707) node[right] {$k=1$};
        \draw[red, mark size=1.5pt,mark=x] plot coordinates {(1,0.707)};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}[scale=5]
        ...
        \draw[mark size=0.4pt,mark=*] plot coordinates {(-1,0)};
        % Points -pi/2
        \node[below] at (0,-1) {\LARGE{$-\frac{\pi}{2}$}};
        \draw[mark size=0.4pt,mark=*] plot coordinates {(0,-1)};
